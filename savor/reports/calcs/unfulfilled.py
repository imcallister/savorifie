...
def unfulfilled(q):
    ...
    usls_expd = sorted(sum([get_lines(sku_map, us) for us in usls], []),
                       key=lambda x: (x['sale_id'], x['inventory_item']))
    us_g = itertools.groupby(usls_expd,
                             lambda x: (x['sale_id'], x['inventory_item']))

    us_d = dict((u, sum(i['quantity__sum'] for i in list(g))) for u, g in us_g)
