...
from .file_models.paypal_payouts import PaypalPayoutCSVModel
...
def upload(request):
    processor = process_Paypal_payouts
    return order_upload(request,
                        processor,
                        label=False)


def process_Paypal_payouts(file_name):
    incoming_name = os.path.join(INCOMING_ROOT, file_name)
    po_records, errors = PaypalPayoutCSVModel.import_data(data=open(incoming_name, 'rU'))
    new_recs_ctr = 0
    exist_recs_ctr = 0
