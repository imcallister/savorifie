...
from .file_models.AMZN_PMTS_payouts import AMZNPmtsPayoutCSVModel
...
def upload(request):
    processor = process_AMZN_PMTS_payouts
    return order_upload(request,
                        processor,
                        label=False)


def process_AMZN_PMTS_payouts(file_name):
    incoming_name = os.path.join(INCOMING_ROOT, file_name)
    po_records, errors = AMZNPmtsPayoutCSVModel.import_data(data=open(incoming_name, 'rU'))
    new_recs_ctr = 0
    exist_recs_ctr = 0
    errors_cnt = len(errors)
    
    # for simplicity only allow file if for a single payor
    paid_thru_id = 'AMZN_PMTS'
    ...
    summary_msg = 'Loaded Amazon Payments payout file: %d new records, %d duplicate records, %d bad rows' \
